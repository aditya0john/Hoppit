...
export default function Cart() {
    let { cartItem } = useCartStore();
    const totalPrice = cartItem.reduce((acc, item) => acc + item.price, 0);

    return (
        <GestureHandlerRootView>
            <SafeAreaView className={`flex-1 bg-[#FFF] p-2 items-center ${cartItem.length === 0 ? "justify-center" : "justify-start"}`} >
                {cartItem.length === 0 ?
                            ...
                            <View className='flex-col gap-4 bg-neutral-200/[0.6] p-4 rounded-2xl w-full mb-10'>
                                ...
                                <View className='gap-1'>
                                    ...
                                    <View className='flex-row px-2 justify-between'>
                                        ...
                                        <Text className='text-black/[0.6]'>Rs 4</Text>
                                ...
                                <View className='flex-row justify-between px-2'>
                                    ...
                                    <Text className='text-black text-xl font-bold'>{totalPrice + 14 + 7 + 4}</Text>
                                </View>
                            </View>
                        </ScrollView>
                    </View>
            ...
            {cartItem.length > 0 &&
                <View className='bg-[#FFF] flex-row items-center justify-between p-2'>
                    <View className='items-center'>
                        ...
                        <Text className='text-md text-blue-500 capitalize'>Cash On Delivery</Text>
                    </View>
                    <TouchableOpacity className='w-56 bg-green-500 p-2 rounded-2xl flex-row items-center justify-center gap-2'>
